...
| R-03 | **Building against both regulatory regimes at once.** Both the US and India constraint sets in `constraint-register.md` bind, roughly doubling the compliance surface carried through design. | High | Medium | **High** | **Accept** (changed from Mitigate at the approval-handoff gate). [Q3 of scope definition] chose both regions at MVP rather than resolving the jurisdiction, so this is a knowing decision to carry both regimes rather than a risk D-01 will close. | Deepak |
...
| R-07 | **Contended capacity stalls the work indefinitely.** No deadline and shared client-delivery capacity together remove both the forcing function and the dedicated attention. | — | — | **Superseded** | **Superseded by `../team-formation/team-assessment.md`.** Scored Medium/Medium before team formation established one engineer on residual time against nine Must-ranked proto-Units. That assessment records the gap as an **issue** — already true — rather than a risk that might occur. The original scoring is noted here rather than silently rewritten, so the change in understanding stays legible. | Deepak |
...
| **R-10** | **The voice channel excludes some patients outright.** A caller who is deaf, hard of hearing, or whose speech the recogniser handles poorly cannot use a voice agent at all. `../rough-mockups/user-flow.md` has no path for them, and [Q5 of rough mockups] deferred the non-voice fallback decision. For a healthcare product the consequence is a patient who cannot reach their clinic. | High | Medium | **High** | Mitigate — decide a non-voice fallback (SMS, web form, or a published direct line that bypasses the agent) before the pilot handles real patients. Added at the approval-handoff gate on the product-lead reviewer's recommendation. | Deepak |
